...
struct cdf_function {
  ...
  Type operator()(Type x){
    ...
    Type inf = 20;
    using romberg::integrate;
    Type F1 = integrate(f, -inf, x, 9 );
    Type F2 = integrate(f, x, inf, 9 );    
    return F1 / (F1 + F2);
  }
};
...
template<class Type>
Type my_transform2(Type &u, Type mu, Type sigma, Type x, Type &f, Type uaux, Type tiny){
  //Type u=parms[0], mu = parms[1], sigma = parms[2], x = parms[3], f=0;
  //vector<Type> ans(2);
  gauss_bernulli<Type> gb = {mu, sigma, x};
  cdf_function<gauss_bernulli<Type> > F = {gb};
  return qnorm(F(u));
...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER(logSigma);
  PARAMETER_VECTOR(u);
  PARAMETER_VECTOR(utilde); // Transformed random effects (input)
  DATA_SCALAR(tiny);
  ...
  if(do_transform){
    for(int i=0;i<obs.size();i++){
      Type tmp;
      if(i==0){
	tmp = my_transform2(u(i), Type(0), sigma, obs(i), f, utilde(i), tiny);
      } else {
	tmp = my_transform2(u(i), phi*u(i-1), sigma * sqrt(1.0 - phi*phi), obs(i), f, utilde(i), tiny);
      }

      f += .5 * pow(utilde(i) - tmp, 2);
